...
if ( !function_exists( 'inkgo_token' ) ) {
function inkgo_token()
{
	...
	if( WC()->session->get('inkgo_campaign_id', '') || isset($_SESSION['inkgo_campaign_id']) || isset($_POST['inkgo_campaign_id']) )
	{
		...
		elseif( isset($_POST['inkgo_campaign_id']) ) $design_id = sanitize_key($_POST['inkgo_campaign_id']);
		$options = array(
			'design_id' 	=> $design_id,
			'timezone' 		=> $timezone,
		...
		{
			$options['layers'] = array_map( 'sanitize_text_field', $_POST['layers'] );
		}
